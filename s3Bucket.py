...
def main():
    ...
    if create_bucket_response['ResponseMetadata']['HTTPStatusCode'] == 200:
        print
        "Successfully created bucket %s" % BUCKET_NAME
    else:
    ...
    print
    "Uploading file %s to bucket %s" % (TEST_FILE, BUCKET_NAME)

    # download file
    client.download_file(BUCKET_NAME, TEST_FILE_KEY, '/tmp/file-from-bucket.txt')
    print
    "Downloading object %s from bucket %s" % (TEST_FILE_KEY, BUCKET_NAME)
    ...
    if delete_object_response['ResponseMetadata']['HTTPStatusCode'] == 204:
        print
        "Successfully deleted object %s from bucket %s" % (TEST_FILE_KEY, BUCKET_NAME)
    else:
    ...
    if delete_bucket_response['ResponseMetadata']['HTTPStatusCode'] == 204:
        print
        "Successfully delete bucket %s" % BUCKET_NAME
    else:
